<template>
  ...
  <div class="interactive-api-container">
    ...
    <div class="main-content">
      ...
      <section id="ws_authorization" class="endpoint-section">
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab1 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X PATCH "https://develop.okd.finance/api/authorization" \
  ...
  -d '{}'</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testWs_authorization" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.ws_authorization" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.ws_authorization.status }}</span>
                  <span class="timestamp">{{ results.ws_authorization.timestamp }}</span>
                  <button @click="copyToClipboard(results.ws_authorization.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.ws_authorization.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.ws_authorization.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.ws_authorization.headers }}</pre>
                  <h5>📦 Body:</h5>
                  <pre class="request-data">{{ results.ws_authorization.body }}</pre>
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.ws_authorization.data }}</pre>
              </div>
            </div>
          </div>
        </div>
...
const ws_authorizationData = reactive({
...
const results = reactive({
  ws_authorization: null
})


const testWs_authorization = async () => {
  try {
    ...
    const fullUrl = `${apiBaseUrl.value}/authorization${queryString}`
