<template>
  ...
  <div class="interactive-api-container">
    ...
    <div class="main-content">
      ...
      <section id="errdesc" class="endpoint-section">
        <div class="endpoint-layout">
          <!-- Documentation -->
          <div class="endpoint-docs">
            <div class="method-header">
              <span class="method-badge get">GET</span>
              <span class="endpoint-path">/errdesc</span>
            </div>
            
            <div class="endpoint-info">
              <h3 class="endpoint-title">Errors list.</h3>
              <p class="endpoint-description">Errors list.</p>
            </div>
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab1 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X GET "https://develop.okd.finance/api/errdesc" \
  ...
  -H "Fingerprint: 1358cd229b6bceb25941e99f4228997f"</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testErrdesc" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.errdesc" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.errdesc.status }}</span>
                  <span class="timestamp">{{ results.errdesc.timestamp }}</span>
                  <button @click="copyToClipboard(results.errdesc.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.errdesc.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.errdesc.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.errdesc.headers }}</pre>
                  
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.errdesc.data }}</pre>
              </div>
            </div>
          </div>
        </div>
...
const testErrdesc = async () => {
  try {
    
    const queryString = ''
    
    
    const fullUrl = `${apiBaseUrl.value}/errdesc${queryString}`
