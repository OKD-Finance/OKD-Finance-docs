<template>
  ...
  -H "Content-Type: application/json"</code></pre>
            ...
            <div class="endpoint-testing">
              ...
              <button 
                @click="errdesc" 
                :disabled="loading['errdesc']"
                class="test-button"
              >
                {{ loading['errdesc'] ? 'Testing...' : 'Test Endpoint' }}
              </button>
              
              <div v-if="error['errdesc']" class="error-message">
                ❌ Error: {{ error['errdesc'] }}
              </div>
              
              <div v-if="responses['errdesc']" class="response-section">
                <h5>📄 Response:</h5>
                <div class="response-data">
                  <pre><code>{{ JSON.stringify(responses['errdesc'], null, 2) }}</code></pre>
                </div>
              </div>
            </div>
          </div>
...
const errdesc = async () => {
  ...
  try {
    ...
    const response = await fetch(`${apiUrl.value}/errdesc`, {
      method: 'GET',
      headers,
    ...
    responses['errdesc'] = data;
    
    if (!response.ok) {
      throw new Error(data.message || 'Request failed');
    }
  } catch (err) {
    error['errdesc'] = err.message;
  } finally {
    loading['errdesc'] = false;
  }
}
</script>
