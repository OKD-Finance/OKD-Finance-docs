...
const testEndpoint1 = async () => {
  try {
    ...
    const fullUrl = `${authValues.apiBaseUrl}/errdesc`
    const headers = {
...
const codeExamples = {
  curl: {
    1: `curl -X GET "https://develop.okd.finance/api/errdesc" \\
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \\
  -H "Content-Type: application/json" \\
  -H "Fingerprint: YOUR_FINGERPRINT" \\
  -d '{}'`
...
func errorslist() error {
    url := "https://develop.okd.finance/api/errdesc"
    
    requestData := ErrorslistRequest{
...
func main() {
    if err := errorslist(); err != nil {
        fmt.Printf("Error: %v\\n", err)
    }
}`
  },
  typescript: {
    1: `interface ErrorslistRequest {

}

async function errorslist(
  baseUrl: string,
  accessToken: string,
  data: ErrorslistRequest
): Promise<any> {
  const response = await fetch(\`\${baseUrl}/errdesc\`, {
    method: 'GET',
    headers: {
...
function errorslist($baseUrl, $accessToken, $data) {
    $url = $baseUrl . '/errdesc';
