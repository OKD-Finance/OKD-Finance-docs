<template>
  ...
  <div class="interactive-api-container">
    ...
    <div class="main-content">
      ...
      <section id="set_profile_flags" class="endpoint-section">
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab1 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X PUT "https://develop.okd.finance/api/user/flags" \
  ...
  -d '{}'</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testSet_profile_flags" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.set_profile_flags" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.set_profile_flags.status }}</span>
                  <span class="timestamp">{{ results.set_profile_flags.timestamp }}</span>
                  <button @click="copyToClipboard(results.set_profile_flags.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.set_profile_flags.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.set_profile_flags.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.set_profile_flags.headers }}</pre>
                  <h5>📦 Body:</h5>
                  <pre class="request-data">{{ results.set_profile_flags.body }}</pre>
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.set_profile_flags.data }}</pre>
              </div>
            </div>
          </div>
        </div>
      </section>
      <section id="ws_notifications" class="endpoint-section">
        <div class="endpoint-layout">
          <!-- Documentation -->
          <div class="endpoint-docs">
            <div class="method-header">
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab2 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X PATCH "https://develop.okd.finance/api/user/notifications" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -H "Fingerprint: 1358cd229b6bceb25941e99f4228997f" \
  -d '{}'</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testWs_notifications" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.ws_notifications" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.ws_notifications.status }}</span>
                  <span class="timestamp">{{ results.ws_notifications.timestamp }}</span>
                  <button @click="copyToClipboard(results.ws_notifications.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.ws_notifications.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.ws_notifications.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.ws_notifications.headers }}</pre>
                  <h5>📦 Body:</h5>
                  <pre class="request-data">{{ results.ws_notifications.body }}</pre>
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.ws_notifications.data }}</pre>
              </div>
            </div>
          </div>
        </div>
      </section>
      <section id="ws_profile" class="endpoint-section">
        <div class="endpoint-layout">
          <!-- Documentation -->
          <div class="endpoint-docs">
            <div class="method-header">
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab3 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X PATCH "https://develop.okd.finance/api/user/profile" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -H "Fingerprint: 1358cd229b6bceb25941e99f4228997f" \
  -d '{}'</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testWs_profile" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.ws_profile" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.ws_profile.status }}</span>
                  <span class="timestamp">{{ results.ws_profile.timestamp }}</span>
                  <button @click="copyToClipboard(results.ws_profile.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.ws_profile.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.ws_profile.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.ws_profile.headers }}</pre>
                  <h5>📦 Body:</h5>
                  <pre class="request-data">{{ results.ws_profile.body }}</pre>
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.ws_profile.data }}</pre>
              </div>
            </div>
          </div>
        </div>
...
const set_profile_flagsData = reactive({

})

const ws_notificationsData = reactive({

})

const ws_profileData = reactive({
...
const results = reactive({
  set_profile_flags: null,
  ws_notifications: null,
  ws_profile: null
})


const testSet_profile_flags = async () => {
  try {
    ...
    const fullUrl = `${apiBaseUrl.value}/user/flags${queryString}`
...
const testWs_notifications = async () => {
  try {
    ...
    const fullUrl = `${apiBaseUrl.value}/user/notifications${queryString}`
    const fingerprint = generateFingerprint()
    const headers = {
      'Authorization': `Bearer ${apiToken.value}`,
      'Content-Type': 'application/json',
...
const testWs_profile = async () => {
  try {
    ...
    const fullUrl = `${apiBaseUrl.value}/user/profile${queryString}`
    const fingerprint = generateFingerprint()
    const headers = {
      'Authorization': `Bearer ${apiToken.value}`,
      'Content-Type': 'application/json',
