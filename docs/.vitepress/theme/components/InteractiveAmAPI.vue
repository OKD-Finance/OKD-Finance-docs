<template>
  ...
  <div class="interactive-api-container">
    ...
    <div class="main-content">
      ...
      <section id="callback" class="endpoint-section">
        <div class="endpoint-layout">
          ...
          <div class="endpoint-docs">
            ...
            <div class="endpoint-info">
              <h3 class="endpoint-title">callback</h3>
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab1 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X POST "https://develop.okd.finance/api/amlbot/callback" \
  ...
  -d '{}'</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testCallback" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.callback" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.callback.status }}</span>
                  <span class="timestamp">{{ results.callback.timestamp }}</span>
                  <button @click="copyToClipboard(results.callback.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.callback.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.callback.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.callback.headers }}</pre>
                  <h5>📦 Body:</h5>
                  <pre class="request-data">{{ results.callback.body }}</pre>
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.callback.data }}</pre>
              </div>
            </div>
          </div>
        </div>
...
const testCallback = async () => {
  try {
    ...
    const fullUrl = `${apiBaseUrl.value}/amlbot/callback${queryString}`
