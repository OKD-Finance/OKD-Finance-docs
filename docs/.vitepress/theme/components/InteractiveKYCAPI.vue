<template>
  ...
  <div class="interactive-api-container">
    ...
    <div class="main-content">
      ...
      <section id="token" class="endpoint-section">
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab1 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X GET "https://develop.okd.finance/api/kyc/token" \
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <div class="form-group">
                <label>kind</label>
                <input v-model="tokenData.kind" type="text" placeholder="Kind of the flow that should be run with the requested access token
" class="test-input" />
              </div>
              
              <button @click="testToken" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.token" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.token.status }}</span>
                  <span class="timestamp">{{ results.token.timestamp }}</span>
                  <button @click="copyToClipboard(results.token.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.token.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.token.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.token.headers }}</pre>
                  
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.token.data }}</pre>
              </div>
            </div>
          </div>
        </div>
      </section>
      <section id="webhook" class="endpoint-section">
        <div class="endpoint-layout">
          <!-- Documentation -->
          <div class="endpoint-docs">
            <div class="method-header">
            ...
            <div class="endpoint-info">
              <h3 class="endpoint-title">webhook</h3>
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab2 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X POST "https://develop.okd.finance/api/kyc/webhook" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  ...
  -d '{}'</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testWebhook" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.webhook" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.webhook.status }}</span>
                  <span class="timestamp">{{ results.webhook.timestamp }}</span>
                  <button @click="copyToClipboard(results.webhook.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.webhook.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.webhook.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.webhook.headers }}</pre>
                  <h5>📦 Body:</h5>
                  <pre class="request-data">{{ results.webhook.body }}</pre>
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.webhook.data }}</pre>
              </div>
            </div>
          </div>
        </div>
...
const testToken = async () => {
  try {
    ...
    if (tokenData.kind) params.append('kind', tokenData.kind)
    const queryString = params.toString() ? '?' + params.toString() : ''
    
    
    const fullUrl = `${apiBaseUrl.value}/kyc/token${queryString}`
...
const testWebhook = async () => {
  try {
    ...
    const fullUrl = `${apiBaseUrl.value}/kyc/webhook${queryString}`
    const fingerprint = generateFingerprint()
    const headers = {
      'Authorization': `Bearer ${apiToken.value}`,
      'Content-Type': 'application/json',
