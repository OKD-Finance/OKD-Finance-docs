<template>
  ...
  <div class="interactive-api-container">
    ...
    <div class="main-content">
      ...
      <section id="ws_unsubscribe" class="endpoint-section">
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab1 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X PATCH "https://develop.okd.finance/api/unsubscribe" \
  ...
  -d '{}'</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testWs_unsubscribe" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.ws_unsubscribe" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.ws_unsubscribe.status }}</span>
                  <span class="timestamp">{{ results.ws_unsubscribe.timestamp }}</span>
                  <button @click="copyToClipboard(results.ws_unsubscribe.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.ws_unsubscribe.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.ws_unsubscribe.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.ws_unsubscribe.headers }}</pre>
                  <h5>📦 Body:</h5>
                  <pre class="request-data">{{ results.ws_unsubscribe.body }}</pre>
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.ws_unsubscribe.data }}</pre>
              </div>
            </div>
          </div>
        </div>
...
const ws_unsubscribeData = reactive({
...
const testWs_unsubscribe = async () => {
  try {
    ...
    const fullUrl = `${apiBaseUrl.value}/unsubscribe${queryString}`
