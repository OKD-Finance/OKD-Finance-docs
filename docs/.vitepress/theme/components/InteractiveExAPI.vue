<template>
  ...
  <div class="interactive-api-container">
    ...
    <div class="main-content">
      ...
      <section id="ws_external" class="endpoint-section">
            ...
            <div class="api-section">
              ...
              <div class="code-examples">
                ...
                <div v-show="activeCodeTab1 === 'cURL'" class="code-block-container">
                  ...
                  <div class="code-block"><pre>curl -X PATCH "https://develop.okd.finance/api/external" \
  ...
  -d '{}'</pre></div>
          ...
          <div class="endpoint-testing">
            ...
            <div class="test-section">
              ...
              <button @click="testWs_external" class="test-btn" :disabled="!apiToken || !apiBaseUrl">
                {{ !apiToken ? '🔒 Enter API Token First' : !apiBaseUrl ? '🌐 Enter API URL First' : '🚀 Test Request' }}
              </button>
              <div v-if="results.ws_external" class="result-container">
                <div class="result-header">
                  <span class="status-badge">{{ results.ws_external.status }}</span>
                  <span class="timestamp">{{ results.ws_external.timestamp }}</span>
                  <button @click="copyToClipboard(results.ws_external.data, $event)" class="copy-btn">📋 Copy Response</button>
                </div>
                <div v-if="results.ws_external.requestUrl" class="request-info">
                  <h5>📤 Actual Request:</h5>
                  <pre class="request-data">{{ results.ws_external.requestUrl }}</pre>
                  <h5>📋 Headers:</h5>
                  <pre class="request-data">{{ results.ws_external.headers }}</pre>
                  <h5>📦 Body:</h5>
                  <pre class="request-data">{{ results.ws_external.body }}</pre>
                </div>
                <h5>📥 Response:</h5>
                <pre class="result-data">{{ results.ws_external.data }}</pre>
              </div>
            </div>
          </div>
        </div>
...
const ws_externalData = reactive({
...
const testWs_external = async () => {
  try {
    ...
    const fullUrl = `${apiBaseUrl.value}/external${queryString}`
